...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (tipo === 'respuestas') {
      // Trae id_registro de la campaña
      ...
      while (true) {
        ...
        for (const r of data) {
          ...
          rows.push({
            ...
            quiere_seguir:        r.paso_4_quiere_seguir,
            motivo:               r.paso_5_motivo,
      ...
      while (true) {
        const { data, error } = await sb
          .from('registros')
          .select(
            'id_registro, nombre_paciente, cedula, correo, tipo_atencion, especialidad, nombre_servicio, centro_medico, correo_estado, correo_enviado_at, correo_abierto_at, correo_click_at, encuesta_completada_at, estado, primer_acceso_at, primer_acceso_dispositivo, primer_acceso_pais, primer_acceso_ciudad'
          )
          .eq('encuesta_campana_id', campanaId)
          .range(from, from + PAGE_SIZE - 1)
